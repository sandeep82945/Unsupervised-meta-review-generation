...
class Prepare:
    ...
    def __init__(self, basepath, p_name, source_file, s_min=1, s_max=20, e_min=2, e_max=1000, t_max=150,
        ...
        # opiniondigest/data/yelp-default
    ...
    def run(self):
        ...
        #returns full review text and its id
    ...
    def _read_reviews(self, source_file):
        ...
        with open(source_file, "r", encoding="utf-8") as file:
            for _, line in enumerate(tqdm(file, desc="reviews")):
                ...
                # tokenise "content" and create "sentences", if absent
                if "sentences" not in review:
                  cont = review["content"].split('. ')
                  A = []
                  for deep in cont:
                    deep = deep+'.'
                    l = deep.split()
                ...
                num_exts += len(exts)
        print("Average number of extractions per review, (avg. no. of opinions per review) : {}".format(num_exts / (0.0 + len(reviews))))
    ...
    def _split_train_dev_test(self):
        ...
        print("num train = ", num_train,"num_dev = ", num_dev, "num_test = ", num_test, "num self ent = ", len(self.entities))

        if self.unk_ratio < 0:
            entities = list(self.entities)
            shuffle(entities)
            train = entities[:num_train + num_dev]
            train = [ele for ele in train if ele not in test_filenames]
            dev = entities[num_train + num_dev:]
            dev = [ele for ele in dev if ele not in test_filenames]
            test = test_filenames
    ...
    def _extraction2input(self, extraction, sep="[SEP]"):
        op = extraction.split(";")[0].split(",")[2:]
        return op[0] + " [SEP] " + op[1] + " [SEP] " + " {} ".format(sep).join([" ".join(e.split(",")[0:2]) for e in extraction.split(";")])
...
if __name__ == "__main__":
    ...
    with open(os.path.join(basepath, "data", configs["p_name"], 'golden_new.json')) as json_file:
      data = json.load(json_file)
    test_filenames = list (data.keys())
    ...
    filename = ["test", "train", "dev"]
    for file in filename:
      fpath = os.path.join(basepath, "data", configs["p_name"], file+".csv")

      def func_ans(reference,phrase):
        rgx = re.compile("(\w[\w']*\w|\w)")
        arr=rgx.findall(phrase)
        s=[]
        for word in arr:
          info=(re.search(word,reference))
          #print(info)
          if(info):
            no=info.span()[0]
            s.append((no,word))
      ...
      def func(reference,phrase):
        phrase = phrase.split('[SEP]')
        ans=""
        for i in range(len(phrase)):
          if(i==len(phrase)-1):
            ans1=(func_ans(reference,phrase[i]))
            if(ans1!=""):
              ans=ans+func_ans(reference,phrase[i])
          elif(i==0 or i==1):
            ans=ans+phrase[i]+"[SEP]"
          else:
            ans1=(func_ans(reference,phrase[i]))
            if(ans1!=""):
              ans=ans+func_ans(reference,phrase[i])+"[SEP]"
        return ans
      ...
      for i in range (shape[0]):
        phrase=(df.iloc[i,4])
        # print("Phrase:",phrase)
        reference=(df.iloc[i,2])
        # print("reference:",reference)
        ans=(func(reference,phrase))
        df.at[i,"input_text"]=ans
